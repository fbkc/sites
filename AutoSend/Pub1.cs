    ...
    public static class Pub1
    {
        ...
        public static bool Publish()
        {
            try
            {
                ...
                while (true)
                {
                    ThreadPool.QueueUserWorkItem(new WaitCallback(PubTitle), MyInfo.qtitle.Dequeue());
                }
            }
            catch (Exception ex)
            {
                log.wlog(ex.ToString(), "", "");
                return false;
            }
            return true;
        }
        private static void PubTitle(object titleInfo)
        {
            titleInfo tInfo = (titleInfo)titleInfo;
